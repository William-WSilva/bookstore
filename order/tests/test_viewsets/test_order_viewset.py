...
class TestOrderViewSet(APITestCase):
    ...
    def setUp(self):
        self.user = UserFactory() # auth
        token = Token.objects.create(user=self.user) # auth
        token.save() # auth
    ...
    def test_order(self):
        token = Token.objects.get(user__username=self.user.username) # auth
        self.client.credentials(HTTP_AUTHORIZATION="Token " + token.key) # auth
